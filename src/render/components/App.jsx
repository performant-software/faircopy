...
export default class App extends Component {
  ...
  onUpdateFairCopyConfig = (e, configUpdate) => {
    ...
    const lockStatus = getConfigStatus(
      configLastAction,
      fairCopyProject.userID
    );
  ...
  componentDidMount() {
    ...
    fairCopy.ipcRegisterCallback(
      "fairCopyConfigCheckedOut",
      this.onFairCopyConfigCheckedOut
    );
    fairCopy.ipcRegisterCallback(
      "updateFairCopyConfig",
      this.onUpdateFairCopyConfig
    );
  ...
  initRootComponent() {
    ...
    if (rootComponent === "MainWindow") {
      fairCopy.ipcRegisterCallback("projectOpened", (event, projectData) =>
        this.openProject(projectData)
      );
      fairCopy.ipcRegisterCallback(
        "projectIncompatible",
        (event, incompatInfo) => this.setState({ ...this.state, incompatInfo })
      );
      ...
      fairCopy.ipcRegisterCallback("imageViewOpened", (event, imageViewData) =>
        this.openImageView(imageViewData)
      );
    } else if (rootComponent === "PreviewWindow") {
      fairCopy.ipcRegisterCallback("previewViewOpened", (event, previewData) =>
        this.openPreviewView(previewData)
      );
    }
  }
  ...
  refreshMainWindow() {
    setTimeout(() => {
      this.mainWindowRef.current.refreshWindow();
    }, 100);
  }
  ...
  render() {
    ...
    if (rootComponent === "MainWindow" && fairCopyProject) {
      ...
      return <ImageWindow imageView={imageView}></ImageWindow>;
    } else if (rootComponent === "PreviewWindow" && previewView) {
      const { resourceEntry, layers, layerID, projectCSS, validModes } =
        previewView;
      return (
        <PreviewWindow
      ...
      return <ProjectWindow appConfig={appConfig}></ProjectWindow>;
    } else return null;
  }
}
