...
import {
  teiDataWordValidator,
  teiDataCountValidator,
  teiDataNumericValidator,
  teiDataProbability,
  teiDataTruthValue,
} from "../../../model/attribute-validators";
...
export default class ParameterDrawer extends Component {
  ...
  getNextErrorStates(elementName, attrName, error) {
    ...
    if (error) {
      if (nextErrorStates[elementName]) {
        if (!nextErrorStates[elementName].includes(attrName))
          nextErrorStates[elementName].push(attrName);
      } else {
        nextErrorStates[elementName] = [attrName];
      }
    } else {
      if (nextErrorStates[elementName]) {
        nextErrorStates[elementName] = nextErrorStates[elementName].filter(
          (a) => a !== attrName
        );
      }
    }
  ...
  changeAttributeHandler = (element, attributeKey) => {
    return (value, error) => {
      ...
      const nextErrorStates = this.getNextErrorStates(
        elementName,
        attributeKey,
        error
      );
      ...
      <ClickAwayListener onClickAway={onClickAway}>
        ...
        >
          <Paper className="attribute-info-popper">
            ...
            <Typography className="attr-description">
              {attrSpec.description}
            </Typography>
            <Typography className="attr-data-type">
              {attrSpec.dataType} {occurrance}
            </Typography>
          </Paper>
        </Popper>
      </ClickAwayListener>
  ...
  renderAttributeField(element, elementName, attrName, value, attrSpec, onChange) {
    ...
    if (readOnly) {
      return <ReadOnlyField attrName={attrName} value={value}></ReadOnlyField>;
    }
    ...
    if (dataType === "teidata.pointer") {
      ...
      return (
        <TEIDataPointerField
          ...
          minOccurs={minOccurs}
          maxOccurs={maxOccurs}
          resourceEntry={teiDocument.resourceEntry}
          parentEntry={teiDocument.parentEntry}
  ...
  renderAttributes(element, elementID, attrState) {
    ...
    return attrFields.length > 0 ? (
      <div>
        <div className="attributeFields">{attrFields}</div>
        {inactiveErrors}
      </div>
    ) : (
  ...
  renderLegendBox(heatmapLevel) {
    ...
    return <div style={style} className="legend-box"></div>;
  }
  ...
  renderIDField(element) {
    ...
    if (element.attrs["xml:id"]) {
      ...
      preExistingCondition = checkID(
        xmlID,
        relativeParentID,
        teiDocument.fairCopyProject.idMap
      )?.errorMessage;
    } else {
      xmlID = "";
  ...
  renderInactiveError(missing) {
    return (
      <Typography key={`inactive-${missing}`} className="missing-element">
        <i className="fas fa-exclamation-triangle fa-sm"></i>
        <b>{missing}</b> is not in the project schema.
      </Typography>
    );
  }
  ...
  renderElement(element, count, key) {
    ...
    const headerAction =
      element instanceof Node ? this.renderIDField(element) : null;
    const inactiveElement =
      fairCopyConfig.elements[elementID] &&
      fairCopyConfig.elements[elementID].active === false;
    ...
    return (
      <Card variant="outlined" className="element" key={key}>
        ...
        {!readOnly && canEditConfig && (
          <CardActions>
            <Button variant="outlined" onClick={openAttributeDialog}>
              Add/Remove Attributes
            </Button>
            {inactiveElement && (
              <Button onClick={onAddToSchema} variant="outlined">
  ...
  render() {
    ...
    for (const element of elements) {
      elementEls.push(
        this.renderElement(element, count, `attr-panel-${count++}`)
      );
    }
