...
export default class TEIDocument {
  ...
  getTEISchema() {
    return this.fairCopyProject
      ? this.fairCopyProject.teiSchema
      : this.teiSchema
  }
  ...
  editorInitialState() {
    ...
    const teiTemplate =
      this.resourceType === 'text'
        ? teiTextTemplate
        : this.resourceType === 'standOff'
          ? teiStandOffTemplate
          : this.resourceType === 'sourceDoc'
            ? teiSourceDocTemplate
            : teiHeaderTemplate('')
  ...
  hasID = (targetID) => {
    const localID = this.parentEntry
      ? this.parentEntry.localID
      : this.resourceEntry.localID
  ...
  onUpdate(transaction) {
    ...
    if (this.isEditable()) {
      ...
      // update the ID Map
      idMap.setResourceMap(
        resourceMap,
        this.resourceEntry.localID,
        this.parentEntry?.localID
      )
      // note unsaved state
      this.changedSinceLastSave =
        this.changedSinceLastSave || transaction.docChanged
    }
  ...
  updateSystemFlags(transaction, onErrorCountChange) {
    ...
    const nextErrors = applySystemFlags(
      teiSchema,
      idMap,
      fairCopyConfig,
      relativeParentID,
  ...
  getVocab(elementName, attrName) {
    ...
    const vocabID =
      fairCopyConfig.elements[elementName].attrState[attrName].vocabID
    const vocab =
      vocabID && fairCopyConfig.vocabs[vocabID]
        ? fairCopyConfig.vocabs[vocabID]
        : []
  ...
  previewDocument() {
    const previewEntry = this.parentEntry
      ? this.parentEntry
      : this.resourceEntry
